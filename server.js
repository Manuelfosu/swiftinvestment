...
// --- Robust load of the built-in SQLite module -------------------------------
// node:sqlite ships with Node 22.5+. On some versions it is gated behind the
// --experimental-sqlite flag. If it fails to load, we relaunch this process
// ONCE with the flag so deployment works no matter how the host runs it.
let sqlite;
try {
  ...
  console.error('[Swift] Could not load node:sqlite. Please use Node.js v22.5 or newer.');
  process.exit(1);
}
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS audit_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER, actor_id INTEGER, action TEXT, detail TEXT,
...
/* ---- Seed a built-in ADMIN account (full access) -------------------------- */
function seedAdmin() {
  const existing = db.prepare('SELECT id FROM users WHERE email = ?').get('admin@swift.io');
  if (existing) { db.prepare("UPDATE users SET role = 'admin' WHERE id = ?").run(existing.id); return existing.id; }
  const { salt, hash } = hashPassword('admin1234');
  const info = db.prepare(
    'INSERT INTO users (name, email, phone, password_hash, salt, role, tier, joined, avatar) VALUES (?,?,?,?,?,?,?,?,?)'
  ).run('Swift Admin', 'admin@swift.io', '', hash, salt, 'admin', 'VIP 1', 'Jun 2026', 'AD');
  const aid = info.lastInsertRowid;
  db.prepare('INSERT INTO wallets (user_id, balance, invested, available) VALUES (?,0,0,0)').run(aid);
  console.log('Seeded ADMIN account -> email: admin@swift.io  password: admin1234');
  return aid;
}
const ADMIN_ID = seedAdmin();

/* ---- Audit log + role helpers --------------------------------------------- */
function logAudit(userId, actorId, action, detail) {
  try {
    db.prepare('INSERT INTO audit_logs (user_id, actor_id, action, detail) VALUES (?,?,?,?)')
      .run(userId == null ? null : userId, actorId == null ? null : actorId, action, detail || '');
...
async function handleApi(req, res, route) {
  ...
  if (route === '/auth/login' && method === 'POST') {
    ...
    logAudit(u.id, u.id, 'auth.login', 'Signed in');
  ...
  if (route === '/wallet/deposit' && method === 'POST') {
    ...
    logAudit(uid, uid, 'wallet.deposit', 'Deposited ' + amount);
  ...
  if (route === '/wallet/withdraw' && method === 'POST') {
    ...
    logAudit(uid, uid, 'wallet.withdraw', 'Requested ' + amount + ' to ' + (b.account || 'account'));
    return sendJson(res, 200, { ok: true, summary: summaryOf(uid) });
  }
  ...
  if (route === '/plans/subscribe' && method === 'POST') {
    ...
    logAudit(uid, uid, 'plans.subscribe', plan.name);
  ...
  if (route.indexOf('/admin/') === 0) {
    if (userRole(uid) !== 'admin') return sendJson(res, 403, { error: 'Admin access required.' });

    if (route === '/admin/stats' && method === 'GET') {
      const users = db.prepare('SELECT COUNT(*) c FROM users').get().c;
      const t = db.prepare('SELECT COALESCE(SUM(balance),0) b, COALESCE(SUM(invested),0) i, COALESCE(SUM(available),0) a FROM wallets').get();
      const pendW = db.prepare("SELECT COUNT(*) c FROM transactions WHERE status='Pending' AND type='Withdrawal'").get().c;
      const pendD = db.prepare("SELECT COUNT(*) c FROM transactions WHERE status='Pending' AND type='Deposit'").get().c;
      const active = db.prepare("SELECT COUNT(*) c FROM subscriptions WHERE status='active'").get().c;
    ...
    if (route === '/admin/users' && method === 'GET') {
      const rows = db.prepare('SELECT u.id, u.name, u.email, u.phone, u.role, u.tier, u.joined, COALESCE(w.balance,0) balance, COALESCE(w.invested,0) invested, COALESCE(w.available,0) available FROM users u LEFT JOIN wallets w ON w.user_id = u.id ORDER BY u.id').all();
      return sendJson(res, 200, rows);
    }
    if (route === '/admin/transactions' && method === 'GET') {
      const rows = db.prepare('SELECT t.id, t.txid, t.date, t.type, t.amount, t.status, u.email AS user FROM transactions t LEFT JOIN users u ON u.id = t.user_id ORDER BY t.id DESC LIMIT 300').all();
      return sendJson(res, 200, rows);
    }
    if (route === '/admin/subscriptions' && method === 'GET') {
      const rows = db.prepare('SELECT s.id, s.name, s.price, s.daily, s.days, s.day_of AS dayOf, s.status, u.email AS user FROM subscriptions s LEFT JOIN users u ON u.id = s.user_id ORDER BY s.id DESC').all();
      return sendJson(res, 200, rows);
    }
    if (route === '/admin/logs' && method === 'GET') {
      const rows = db.prepare('SELECT l.id, l.action, l.detail, l.time, u.email AS user, a.email AS actor FROM audit_logs l LEFT JOIN users u ON u.id = l.user_id LEFT JOIN users a ON a.id = l.actor_id ORDER BY l.id DESC LIMIT 200').all();
      return sendJson(res, 200, rows);
    }
    if (route === '/admin/wallet/adjust' && method === 'POST') {
      const b = await readBody(req);
      const target = Number(b.userId); const amt = Number(b.amount);
      if (!target || !amt) return sendJson(res, 400, { error: 'Provide userId and a non-zero amount.' });
      const w = db.prepare('SELECT user_id FROM wallets WHERE user_id = ?').get(target);
      if (!w) return sendJson(res, 404, { error: 'User wallet not found.' });
      db.prepare('UPDATE wallets SET balance = balance + ?, available = available + ? WHERE user_id = ?').run(amt, amt, target);
      db.prepare('INSERT INTO transactions (user_id, txid, date, type, amount, status) VALUES (?,?,?,?,?,?)').run(target, nextTxId(), today(), amt > 0 ? 'Credit' : 'Debit', Math.abs(amt), 'Completed');
      logAudit(target, uid, 'admin.wallet.adjust', (amt > 0 ? '+' : '') + amt + (b.note ? (' — ' + b.note) : ''));
    ...
    if (route === '/admin/transaction/status' && method === 'POST') {
      ...
      if (tx.type === 'Withdrawal' && tx.status === 'Pending' && action === 'Rejected') {
        db.prepare('UPDATE wallets SET balance = balance + ?, available = available + ? WHERE user_id = ?').run(tx.amount, tx.amount, tx.user_id);
      }
      ...
      logAudit(tx.user_id, uid, 'admin.tx.status', tx.txid + ' -> ' + newStatus);
      return sendJson(res, 200, { ok: true });
    }
    if (route === '/admin/plan/cancel' && method === 'POST') {
      const b = await readBody(req);
      const sub = db.prepare('SELECT * FROM subscriptions WHERE id = ?').get(Number(b.id));
      ...
      logAudit(sub.user_id, uid, 'admin.plan.cancel', sub.name);
      return sendJson(res, 200, { ok: true });
    }
    if (route === '/admin/user/role' && method === 'POST') {
      ...
      logAudit(target, uid, 'admin.user.role', role);
      return sendJson(res, 200, { ok: true });
    }
 ...
 * 6. START THE SERVER
 * ===========================================================================*/
// Keep the service alive even if an unexpected error slips through.
process.on('uncaughtException', (e) => console.error('[Swift] uncaughtException:', e && e.message ? e.message : e));
process.on('unhandledRejection', (e) => console.error('[Swift] unhandledRejection:', e && e.message ? e.message : e));
